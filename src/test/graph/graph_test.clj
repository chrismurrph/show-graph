...
(deftest nodes-in-graphs-edges
  (= #{:12 :11 :10 :4 :7 :1 :8 :9 :2 :5 :3 :6}
     (is = (graph/nodes-in-edges example/nodes-graph))))

(deftest all-example-graphs-are-graphs
  (is = (every? graph/graph? [example/simple-graph example/full-graph example/nodes-graph example/unreachable-nodes-graph])))
